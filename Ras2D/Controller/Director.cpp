...
namespace Ras2D
{
	static Director* sharedInstance = nullptr;
	...
	std::string Director::LoadShaderFromFile(const std::string &fileName) const
	{
		//load file to string
	...
	bool Director::CheckForShaderError(const GLuint shader) const
	{
		int params = -1;
	...
	bool Director::CheckForShaderLinkErrors(const GLuint shaderProgram) const
	{
		int params = -1;
	...
	GLuint Director::LoadShader(const std::string &fileName) const
	{
	...
	bool Director::Update(std::vector<bool> &keys)
	{
		// update other events like input handling
		glfwPollEvents();

		//TODO This needs offloaded to the scheduler
	...
	bool Director::IsShaderProgramValid(const GLuint shader) const
	{
