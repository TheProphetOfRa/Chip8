...
namespace Chip8
{
    ...
    void CPU::Execute(void)
    {
        ...
        instructionTable[_opcode & 0xF000](_opcode);
        return;
    ...
    void CPU::setupOpCodes()
    {
        ...
        instructionTable[0x1000] = [&](unsigned short code) {
            _pc = _opcode & 0x0FFF;
        };
        
        //2NNN call subroutine at NNN
        instructionTable[0x2000] = [&](unsigned short code) {
        ...
        instructionTable[0x3000] = [&](unsigned short code) {
        ...
        instructionTable[0x4000] = [&](unsigned short code) {
            if (_v[(_opcode & 0x0F00) >> 8] != (_opcode & 0x00FF))
                _pc += 4;
            else
                _pc += 2;
        ...
        instructionTable[0x5000] = [&](unsigned short code) {
        ...
        //6XNN Sets v[X] to NN
        instructionTable[0x6000] = [&](unsigned short code) {
            _v[(_opcode & 0x0F00) >> 8] = _opcode & 0x00FF;
            _pc+=2;
        };
        
        //7XNN Adds NN to v[X]
        instructionTable[0x7000] = [&](unsigned short code) {
            _v[(_opcode & 0x0F00) >> 8] += _opcode & 0x00FF;
            _pc+=2;
        };
        
        instructionTable[0x8000] = [&](unsigned short code) {
        ...
        instructionTable[0x9000] = [&](unsigned short code) {
            if (_v[(_opcode & 0x0F00) >> 8] != _v[(_opcode & 0x00F0) >> 4])
                _pc += 4;
            else
                _pc += 2;
        ...
        instructionTable[0xA000] = [&](unsigned short code) {
            _I = _opcode & 0x0FFF;
            _pc += 2;
        };
        
        instructionTable[0xB000] = [&](unsigned short code) {
            _pc = (_opcode & 0x0FFF) + _v[0];
        };
        
        instructionTable[0xC000]  =[&](unsigned short code) {
            _v[(_opcode & 0x0F00) >> 8] = (rand() % 0xFF) & (_opcode & 0x00FF);
            _pc+=2;
        };
        
        instructionTable[0xD000] = [&](unsigned short code) {
            {
        ...
        instructionTable[0xE000] = [&](unsigned short code) {
        ...
        instructionTable[0xF000] = [&](unsigned short code) {
            switch(_opcode & 0x00FF)
            {
